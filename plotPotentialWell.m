...
%% data
dat = load('potentialWellGauDat.mat'); % Gaussian noise
eta = dat.eta;
b = dat.b;
recordIterationNumberGau = dat.recordIterationNumber;
 
%dat = load('potentialWellUniDat.mat'); % Uniform noise
%recordIterationNumberUni = dat.recordIterationNumber;

%dat = load('potentialWellBerDat.mat'); % Bernoulli noise
%recordIterationNumberBer = dat.recordIterationNumber;

%dat = load('potentialWellSphDat.mat'); % Spherical Gaussian noise
...
h = legend('$\gamma =-2$', '$\gamma = -1$', '$\gamma = -0.5$', '$\gamma = -0.25$');
...
saveas(gcf,['./','potentialWellGauFig','.pdf']);
...
saveas(gcf,['./','potentialWellUniFig','.pdf']);
...
saveas(gcf,['./','potentialWellBerFig','.pdf']);
...
saveas(gcf,['./','potentialWellSphFig','.pdf']);
